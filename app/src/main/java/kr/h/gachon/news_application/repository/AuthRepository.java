...
public class AuthRepository {
    ...
    public AuthRepository(Context context) {this.api = RetrofitClient.getApiService(context);}

    public void login(String userName, String password, Callback<LoginResponse> cb){
        api.login(new LoginRequest(userName, password)).enqueue(cb);
    }

    public void register(String userName, String password, String email, Callback<String> cb){
        api.register(new RegisterRequest(userName, password, email)).enqueue(cb);
    }
